...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  with config do
    ...
    install_MacTeX                                      # used to compile TeX files into PDFs
    install_editor
...
class VagrantHelper
    ...
    def setup_synced_folder(synced_folder)
      create_folder_if_missing(synced_folder[:host])
    ...
    def install_MacTeX
      say "Installing MacTeX (2.3GB)"
      install_pkg "http://mirror.ctan.org/systems/mac/mactex/MacTeX.pkg"
    end
